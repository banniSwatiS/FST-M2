-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/siva/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/siva/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/siva/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/siva/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/siva/outputs' USING PigStorage('\t');